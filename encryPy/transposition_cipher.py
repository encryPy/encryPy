...
class transpose:
    ...
    def encode(self, key, message)->str:
        ...
        for index in sorted(self.order.keys()):
            for part in transpose._split_len(message, len(key)):
                try:
    ...
    def encrypt(self, key, message) -> str:
        ...
        Example
        -------
        >>> from encrypy.transposition_cipher import transpose
        >>> encrypted_data = transpose.encrypt(key='TESGIN', message='Encrypt Python')
    ...
    def decrypt(self, key, cipher) -> str:
        ...
        -------
        >>> from encrypy.transposition_cipher import transpose
        >>> text = transpose.decrypt(key='TESGIN', message=f'{encrypted_data}')
